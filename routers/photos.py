...
@router.get("/photos/thumb/{key:path}")
async def api_photos_thumb(request: Request, key: str, size: int = 100):
    """Return a small thumbnail for fast color extraction. Max 200px."""
    eff_uid, req_uid = resolve_workspace_uid(request)
    if not eff_uid or not req_uid:
        return JSONResponse({"error": "Unauthorized"}, status_code=401)
    if not has_role_access(req_uid, eff_uid, 'gallery'):
    ...
    size = min(max(size, 50), 200)  # Clamp between 50-200
    origin = request.headers.get('origin', '*')
    cors_headers = {
        "Access-Control-Allow-Origin": origin,
        "Access-Control-Allow-Methods": "GET, OPTIONS",
    ...
    try:
        ...
        return Response(content=buf.getvalue(), media_type="image/jpeg", headers=cors_headers)
    except Exception as ex:
        logger.exception(f"Thumb error for {key}: {ex}")
        return JSONResponse({"error": "Failed"}, status_code=500, headers=cors_headers)


@router.options("/photos/thumb/{key:path}")
async def api_photos_thumb_options(request: Request, key: str):
    """CORS preflight for thumb endpoint."""
    origin = request.headers.get('origin', '*')
    return Response(
        status_code=200,
        headers={
